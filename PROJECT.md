...
| **Balances and Bankroll Center** | `backend/routes/dashboard_routes.py` `GET /balances` handler; `backend/routes/transactions_routes.py` for manual bankroll logs; `backend/services/balance_stats.py` for sportsbook balance calculation; transaction/bet mutation handlers for invalidation inputs; `frontend/src/lib/api.ts`, `frontend/src/lib/hooks.ts`, `frontend/src/components/bankroll/`, and settings/analytics bankroll UI consumers |
