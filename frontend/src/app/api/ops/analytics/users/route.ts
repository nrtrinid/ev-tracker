...
export async function GET(request: Request) {
  ...
  try {
    ...
    const endpoint = `${backendBaseUrl.replace(/\/$/, "")}/api/ops/analytics/users${qs ? `?${qs}` : ""}`;
    ...
    console.error("Analytics users bridge error:", error);
    return NextResponse.json(
      { error: "Failed to fetch analytics users drilldown" },
