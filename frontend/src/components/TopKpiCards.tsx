...
function BeatCloseValue({
  ...
  if (beatClosePct !== null && beatClosePct !== undefined) {
    const color =
      beatClosePct >= 55 ? "text-color-profit-fg" : beatClosePct >= 45 ? "text-foreground" : "text-color-loss-fg";
    return (
      <>
  ...
  if (avgClvPct !== null && avgClvPct !== undefined) {
    const color =
      avgClvPct >= 1.5 ? "text-color-profit-fg" : avgClvPct >= 0 ? "text-muted-foreground" : "text-color-loss-fg";
    return (
      <>
        <p className={cn("text-xl sm:text-2xl font-bold font-mono tracking-tight", color)}>
          {avgClvPct >= 0 ? "+" : ""}
    ...
    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 group">
        ...
        <CardContent className="p-4 flex flex-col items-center justify-center">
          ...
          <p
            className={cn(
              ...
              netProfit >= 0 ? "text-color-profit-fg" : "text-color-loss-fg",
            )}
          >
            {netProfit >= 0 ? "+" : ""}
      ...
      <Card className="card-hover">
        <CardContent className="p-4 flex flex-col items-center justify-center">
          ...
          <p className="text-xl sm:text-2xl font-bold font-mono tracking-tight text-primary">
            {expectedProfit >= 0 ? "+" : ""}
